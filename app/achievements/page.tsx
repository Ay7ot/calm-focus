...
export default async function AchievementsPage() {
  ...
  const nextMilestone = milestones?.find(m => m.session_threshold > (totalSessions || 0))
      ...
      <header className="h-16 border-b border-border bg-surface-elevated sticky top-0 z-20">
        <div className="h-full mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-2 sm:gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base sm:text-lg font-bold text-on-surface">Achievements</h1>
            <p className="text-on-surface-secondary text-xs hidden sm:block">Track your progress and unlock badges</p>
          </div>
